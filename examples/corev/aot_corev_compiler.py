...
def get_args():
    ...
    parser.add_argument(
        ...
        action="store",
        required=False,
        help="Filename (if .pte is used) or a folder for outputs, if not specified the default is to place files in cwd.",
    )
    parser.add_argument(
        "--system_config",
        required=False,
...
if __name__ == "__main__":
    ...
    if args.output is not None:
        if args.output.endswith(".pte"):
            # --output is a pte filename use it as output name
